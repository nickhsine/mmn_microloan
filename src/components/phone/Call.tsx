...
export const Call = forwardRef<HTMLDivElement, CallProps>(({ 
  ...
  const combinedRef = useCallback((node: HTMLDivElement | null) => {
    if (node) { (callRef as any).current = node; }
  ...
  useGSAP(() => {
    ...
    ScrollTrigger.create({
      ...
      markers: markers,
      id: 'call-trigger',
      onEnter: () => {gsap.fromTo(callRef.current, { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.5, });},
      onLeave: () => {gsap.fromTo(callRef.current, { opacity: 1, scale: 1 }, { opacity: 0, scale: 0.9, duration: 0.5, });},
      onEnterBack: () => {gsap.fromTo(callRef.current, { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.25, });},
      onLeaveBack: () => {gsap.fromTo(callRef.current, { opacity: 1, scale: 1 }, { opacity: 0, scale: 0.9, duration: 0.25, });},
      onUpdate: (self) => {
