...
export const MessagesApp = forwardRef<HTMLDivElement, MessagesAppProps>(({ 
  ...
  const combinedRef = useCallback((node: HTMLDivElement | null) => {
    if (node) { (messagesAppRef as any).current = node; }
  ...
  useGSAP(() => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {gsap.fromTo(messagesAppRef.current, 
        { opacity: 0, scale: 0.9 }, 
        { opacity: 1, scale: 1, duration: 0.5, });
      },
      onLeave: () => {gsap.fromTo(messagesAppRef.current, 
        { opacity: 1, scale: 1 }, 
        { opacity: 0, scale: 0.9, duration: 0.5, });
      },
      onEnterBack: () => {gsap.fromTo(messagesAppRef.current, 
        { opacity: 0, scale: 0.9 }, 
        { opacity: 1, scale: 1, duration: 0.25, });
      },
      onLeaveBack: () => {gsap.fromTo(messagesAppRef.current, 
        { opacity: 1, scale: 1 }, 
